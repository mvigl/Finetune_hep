...
checkpoint="/raven/u/mvigl/public/run/Finetune_Xbb_hl/models/Finetune_Xbb_hl_lr0.001_bs256_subset1.0_epoch_1_Val_loss_0.1018531545996666.pt"
mess="Finetune_Xbb_hl"
...
subset=0.0001
nohup sbatch --job-name="$mess${subset}" --time="${JOBTIME}" single_job_LoRa.sbatch "$config" "$checkpoint" "$mess" "$data" "$subset" "$data_val" "$out" "$bs" "$se" >> "$LOG_FILE" 2>> "$ERR_FILE" &
disown -h
