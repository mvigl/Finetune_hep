...
JOBTIME="23:55:00" # Wall clock limit (max. is 24 hours)
TLIMIT=23.6
NUMJOBS=1
...
mess="latent_scores"
use_hlf=""
save_representaions="--save_representaions"
ishead=""
Xbb=""
repDim=128

data="/raven/u/mvigl/public/Finetune_hep/config/train_list.txt"
out="/raven/u/mvigl/public/run/latent_scores_train"
...
out="/raven/u/mvigl/public/run/latent_scores_test"
nohup sbatch --job-name="$mess" --time="${JOBTIME}" single_job_save_scores.sbatch "$config" "$checkpoint" "$mess" "$use_hlf" "$out" "$ishead" "$Xbb" "$repDim" "$data" "$out" >> "$LOG_FILE" 2>> "$ERR_FILE" &
disown -h

data="/raven/u/mvigl/public/Finetune_hep/config/val_list.txt"
out="/raven/u/mvigl/public/run/latent_scores_val"
nohup sbatch --job-name="$mess" --time="${JOBTIME}" single_job_save_scores.sbatch "$config" "$checkpoint" "$mess" "$use_hlf" "$out" "$ishead" "$Xbb" "$repDim" "$data" "$out" >> "$LOG_FILE" 2>> "$ERR_FILE" &
disown -h
exit 0
