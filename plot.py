...
ParTevent_model.load_state_dict(torch.load('../../Finetune_hep/models/ParTevent/ParTevent_hl6_nodes128_nj2_lr4e-05_bs512_WparT_training_0.pt'))
...
for i in range(len(tpr)):
    plt.plot(tpr[i], 1/fpr[i], lw=0.8, label=(f'{models_name[i]} ' + "AUC = {:.3f}%".format(auc(fpr[i],tpr[i])*100)), color=next(color))
