...
Aux3_1_model = ParT_mlp_aux.get_model(data_config,for_inference=False)  
...
Aux_model.load_state_dict(torch.load(f'../../Finetune_hep/models/Aux/Aux_hl3_nodes128_nj2_lr4e-05_bs512_alpha0.5_WparT_hlXbb3_training_0.pt'))
...
Aux3_1_model.load_state_dict(torch.load(f'../../Finetune_hep/models/Aux/Aux_hl3_nodes128_nj2_lr4e-05_bs512_alpha1.0_WparT_hlXbb3_training_0.pt'))
...
yi = ParT_mlp_aux.get_preds(Aux3_1_model,Aux_test,evts_test_aux,device)[1]
fpr_i, tpr_i, threshold_i = roc_curve(labels_test, yi,drop_intermediate=False)
fpr.append(fpr_i)
tpr.append(tpr_i)
threshold.append(threshold_i)
...
ax.plot(tpr[4], 1/fpr[4], lw=0.8, label=('Aux3_05 AUC = {:.2f}%'.format(auc(fpr[4],tpr[4])*100)), color=next(color))
ax.plot(tpr[5], 1/fpr[5], lw=0.8, label=('Aux3 AUC = {:.2f}%'.format(auc(fpr[5],tpr[5])*100)), color=next(color))
ax.plot(tpr[6], 1/fpr[6], lw=0.8, label=('Aux3_1 AUC = {:.2f}%'.format(auc(fpr[6],tpr[6])*100)), color=next(color))
