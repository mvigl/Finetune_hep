...
for i in range(len(sizes)):
    ...
    target_ParTevent = np.array(target_ParTevent).reshape(-1)
    yi_ParTevent = np.array(yi_ParTevent).reshape(-1)
    target_mlpHlXbb = np.array(target_mlpHlXbb).reshape(-1)
    yi_mlpHlXbb = np.array(yi_mlpHlXbb).reshape(-1)    
