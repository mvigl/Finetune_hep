...
class ParticleTransformerWrapper(nn.Module):
    def __init__(self, **kwargs) -> None:
        ...
        self.fcXbb = make_mlp(in_dim,out_features=128,nlayer = 3)
    ...
    def forward(self, points, features, lorentz_vectors, mask):
        ...
        output_Xbb = self.fcXbb(x_cls)
        output_parT = torch.reshape(x_cls,(-1,2*128))
        output_evt = self.fc(output_parT)
        return output_evt, output_Xbb
...
def infer(model,data,train_batch,device):
    ...
    preds, preds_Xbb = model(pf_points,pf_features,pf_vectors,pf_mask)
    return preds.reshape(len(train_batch)), preds_Xbb.reshape(len(train_batch)*2)
...
def train_step(model,data,labels,opt,loss_fn,train_batch,alpha,device):
    model.train()
    preds, preds_Xbb = infer(model,data,train_batch,device)
    target = torch.tensor(labels[train_batch]).float().to(device)
    target_Xbb = torch.tensor(np.reshape(data['label'][train_batch],(-1))).float().to(device)
    loss_evt = loss_fn(preds,target)
    loss_Xbb = loss_fn(preds_Xbb,target_Xbb)
    loss = (loss_evt+alpha*loss_Xbb)
    loss.backward()
    opt.step()
    opt.zero_grad()
    return {'loss': float(loss)}

def eval_fn(model,labels, loss_fn,data,train_set,validtion_set,alpha,device):
    with torch.no_grad():
        model.eval()
        ...
        for i in range(len(ix_train)):
            preds_train_i, preds_Xbb_train_i= infer_val(model,data,ix_train[i],device)
            if i==0:
                preds_train = preds_train_i.detach().cpu().numpy()
                preds_Xbb_train = preds_Xbb_train_i.detach().cpu().numpy()
            else:    
                preds_train = np.concatenate((preds_train,preds_train_i.detach().cpu().numpy()),axis=0)
                preds_Xbb_train = np.concatenate((preds_Xbb_train,preds_Xbb_train_i.detach().cpu().numpy()),axis=0)
        preds_train = torch.tensor(preds_train).float().to(device)
        preds_Xbb_train = torch.tensor(preds_Xbb_train).float().to(device)

        ix_val = np.array_split(validtion_set,int(len(validtion_set)/512))

        for i in range(len(ix_val)):
            preds_val_i, preds_Xbb_val_i = infer_val(model,data,ix_val[i],device)
            if i==0:
                preds_val = preds_val_i.detach().cpu().numpy()
                preds_Xbb_val = preds_Xbb_val_i.detach().cpu().numpy()
            else:    
                preds_val = np.concatenate((preds_val,preds_val_i.detach().cpu().numpy()),axis=0) 
                preds_Xbb_val = np.concatenate((preds_Xbb_val,preds_Xbb_val_i.detach().cpu().numpy()),axis=0)       
        preds_val = torch.tensor(preds_val).float().to(device)
        preds_Xbb_val = torch.tensor(preds_Xbb_val).float().to(device)
        
        target_train = torch.tensor(labels[train_set]).float().to(device)
        target_Xbb_train = torch.tensor(np.reshape(data['label'][train_set],(-1))).float().to(device)
        train_loss = loss_fn(preds_train,target_train)
        train_Xbb_loss = loss_fn(preds_Xbb_train,target_Xbb_train)
        target_val = torch.tensor(labels[validtion_set]).float().to(device)
        target_Xbb_val = torch.tensor(np.reshape(data['label'][validtion_set],(-1))).float().to(device)
        test_loss = loss_fn(preds_val,target_val)
        test_Xbb_loss = loss_fn(preds_Xbb_val,target_Xbb_val)
        print(f'train_loss: {float(train_loss+alpha*train_Xbb_loss)} | test_loss: {float(test_loss+alpha*test_Xbb_loss)}')
        return {'test_loss': float(test_loss+alpha*test_Xbb_loss), 'train_loss': float(train_loss+alpha*train_Xbb_loss)}
...
def train_loop(model, data, labels, device,experiment, path, config):
    ...
    for epoch in range (0,config['epochs']):
        ...
        for i, train_batch in enumerate( train_batches ):
            ...
            report = train_step(model, data, labels, opt, loss_fn,train_batch,alpha,device)
        evals.append(eval_fn(model,labels, loss_fn,data,train_set,validtion_set,alpha,device) )    
...
def get_preds(model,data,evts,device):
    ...
    for i in range(len(ix)):
        preds_i, preds_Xbb_i = infer_val(model,data,ix[i],device)
        if i==0:
            yi_model = preds_i.detach().cpu().numpy()
            yi_Xbb_model = preds_Xbb_i.detach().cpu().numpy()
        else:    
            yi_model = np.concatenate((yi_model,preds_i.detach().cpu().numpy()),axis=0)
            yi_Xbb_model = np.concatenate((yi_Xbb_model,preds_Xbb_i.detach().cpu().numpy()),axis=0)
    return yi_model, yi_Xbb_model