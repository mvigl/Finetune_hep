...
if (checkpoint=='no') or (subset) or (check_message=='no'): 
    ...
    if ((modeltype in ['ParTevent','ParTevent_frozen','ParTXbb','Aux','ParTevent_Hl','ParTevent_Xbb_Hl']) and (subset==False)):
        for i in range(10):
...
if modeltype not in ['mlpLatent','LatentXbb','LatentXbb_Aux','mlpXbb','mlpLatentHl','mlpHlXbb']:
